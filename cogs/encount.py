...
class RecruitView(View):
    ...
    async def apply_button(self, interaction: discord.Interaction, button: Button):
        ...
        # --- ✅ 同陣営のみ立候補可能チェック ---
        if not any(r.id == target_role_id for r in applicant.roles):
            await interaction.response.send_message(
                "⚠️ あなたはこの募集の対象ロールを持っていません！",
                ephemeral=True
            )
            print(f"🚫 {applicant.display_name} は対象ロールを持っていないため拒否されました。")
            return

        # --- 🔹 応答（ユーザーに通知）
        await interaction.response.send_message(
            f"🙋‍♀️ {applicant.display_name} さんが立候補しました！",
            ephemeral=True
        )
        ...
        embed = discord.Embed(
            ...
            description=f"{applicant.mention}（{applicant.display_name}）が {owner.mention} に立候補しました。"
        )
        await log_ch.send(embed=embed)
        print("🪵 ログ送信完了")
...
class EncountCog(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.cleanup_empty_vcs.start()  # ← ここで start できるようにする
